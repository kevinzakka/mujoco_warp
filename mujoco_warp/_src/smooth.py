...
from mujoco_warp._src.support import mask_args
...
@cache_kernel
def _kinematics_branch(use_mask: bool):
...
@cache_kernel
def _compute_body_inertial_frames(use_mask: bool):
  @wp.kernel(module="unique", enable_backward=False)
  def kernel(
    mask_in: wp.array(dtype=bool),
    # Model:
...
@cache_kernel
def _compute_body_matrices(use_mask: bool):
  @wp.kernel(module="unique", enable_backward=False)
  def kernel(
    mask_in: wp.array(dtype=bool),
    # Data in:
...
@cache_kernel
def _geom_local_to_global(use_mask: bool):
  @wp.kernel(module="unique", enable_backward=False)
  def kernel(
    mask_in: wp.array(dtype=bool),
    # Model:
...
@cache_kernel
def _site_local_to_global(use_mask: bool):
  @wp.kernel(module="unique", enable_backward=False)
  def kernel(
    mask_in: wp.array(dtype=bool),
    # Model:
...
@cache_kernel
def _flex_vertices(use_mask: bool):
  @wp.kernel(module="unique", enable_backward=False)
  def kernel(
    mask_in: wp.array(dtype=bool),
    # Model:
...
@event_scope
def kinematics(m: Model, d: Data, mask=None):
  ...
  """
  use_mask, mask = mask_args(mask)
  wp.launch(
    _kinematics_branch(use_mask),
  ...
  wp.launch(
    _compute_body_matrices(use_mask),
  ...
  wp.launch(
    _compute_body_inertial_frames(use_mask),
    dim=(d.nworld, m.nbody),
    inputs=[mask, m.body_ipos, m.body_iquat, d.xpos, d.xquat],
    outputs=[d.xipos, d.ximat],
  )

  wp.launch(
    _geom_local_to_global(use_mask),
    dim=(d.nworld, m.ngeom),
    inputs=[mask, m.body_rootid, m.body_weldid, m.body_mocapid, m.geom_bodyid, m.geom_pos, m.geom_quat, d.xpos, d.xquat],
    outputs=[d.geom_xpos, d.geom_xmat],
  )

  wp.launch(
    _site_local_to_global(use_mask),
    dim=(d.nworld, m.nsite),
    inputs=[mask, m.site_bodyid, m.site_pos, m.site_quat, d.xpos, d.xquat],
    outputs=[d.site_xpos, d.site_xmat],
  )


@event_scope
def flex(m: Model, d: Data, mask=None):
  use_mask, mask = mask_args(mask)
  wp.launch(_flex_vertices(use_mask), dim=(d.nworld, m.nflexvert), inputs=[mask, m.flex_vertbodyid, d.xpos], outputs=[d.flexvert_xpos])
  wp.launch(
    _flex_edges(use_mask),
...
@cache_kernel
def _subtree_com_init(use_mask: bool):
  @wp.kernel(module="unique", enable_backward=False)
  def kernel(
    mask_in: wp.array(dtype=bool),
    # Model:
...
@cache_kernel
def _subtree_com_acc(use_mask: bool):
  @wp.kernel(module="unique", enable_backward=False)
  def kernel(
    mask_in: wp.array(dtype=bool),
    # Model:
...
@cache_kernel
def _subtree_div(use_mask: bool):
  @wp.kernel(module="unique", enable_backward=False)
  def kernel(
    mask_in: wp.array(dtype=bool),
    # Model:
...
@event_scope
def com_pos(m: Model, d: Data, mask=None):
  ...
  """
  use_mask, mask = mask_args(mask)
  wp.launch(_subtree_com_init(use_mask), dim=(d.nworld, m.nbody), inputs=[mask, m.body_mass, d.xipos], outputs=[d.subtree_com])

  for i in reversed(range(len(m.body_tree))):
    body_tree = m.body_tree[i]
    wp.launch(
      _subtree_com_acc(use_mask),
      dim=(d.nworld, body_tree.size),
      inputs=[mask, m.body_parentid, d.subtree_com, body_tree],
      outputs=[d.subtree_com],
    )

  wp.launch(_subtree_div(use_mask), dim=(d.nworld, m.nbody), inputs=[mask, m.body_subtreemass, d.subtree_com], outputs=[d.subtree_com])
  wp.launch(
    _cinert(use_mask),
    dim=(d.nworld, m.nbody),
    inputs=[mask, m.body_rootid, m.body_mass, m.body_inertia, d.xipos, d.ximat, d.subtree_com],
    outputs=[d.cinert],
  )
  wp.launch(
    _cdof(use_mask),
    dim=(d.nworld, m.njnt),
    inputs=[mask, m.body_rootid, m.jnt_type, m.jnt_dofadr, m.jnt_bodyid, d.xmat, d.xanchor, d.xaxis, d.subtree_com],
    outputs=[d.cdof],
  )


@cache_kernel
def _cam_local_to_global(use_mask: bool):
  @wp.kernel(module="unique", enable_backward=False)
  def kernel(
    mask_in: wp.array(dtype=bool),
    # Model:
...
@cache_kernel
def _light_local_to_global(use_mask: bool):
  @wp.kernel(module="unique", enable_backward=False)
  def kernel(
    mask_in: wp.array(dtype=bool),
    # Model:
...
@event_scope
def camlight(m: Model, d: Data, mask=None):
  ...
  wp.launch(
    _light_local_to_global(use_mask),
...
@cache_kernel
def _crb_accumulate(use_mask: bool):
  @wp.kernel(module="unique", enable_backward=False)
  def kernel(
    mask_in: wp.array(dtype=bool),
    # Model:
...
@event_scope
def crb(m: Model, d: Data, mask=None):
  ...
  if use_mask:
    wp.launch(
      support.masked_copy_vec10(True),
      dim=d.crb.shape,
      inputs=[mask, d.crb, d.cinert],
    )
  else:
  ...
  for i in reversed(range(len(m.body_tree))):
    body_tree = m.body_tree[i]
    wp.launch(_crb_accumulate(use_mask), dim=(d.nworld, body_tree.size), inputs=[mask, m.body_parentid, d.crb, body_tree], outputs=[d.crb])

  if use_mask:
    wp.launch(
      support.masked_zero_3d(True),
      dim=d.qM.shape,
      inputs=[mask],
      outputs=[d.qM],
    )
  ...
  if m.is_sparse:
    wp.launch(
      _qM_sparse(use_mask),
      dim=(d.nworld, m.nv),
      inputs=[mask, m.dof_bodyid, m.dof_parentid, m.dof_Madr, m.dof_armature, d.cdof, d.crb],
      outputs=[d.qM],
    )
  else:
    wp.launch(
      _qM_dense(use_mask), dim=(d.nworld, m.nv), inputs=[mask, m.dof_bodyid, m.dof_parentid, m.dof_armature, d.cdof, d.crb], outputs=[d.qM]
    )


@cache_kernel
def _tendon_armature(use_mask: bool):
  @wp.kernel(module="unique", enable_backward=False)
  def kernel(
    mask_in: wp.array(dtype=bool),
    # Model:
...
@event_scope
def tendon_armature(m: Model, d: Data, mask=None):
  """Add tendon armature to qM."""
  use_mask, mask = mask_args(mask)
  wp.launch(
    _tendon_armature(use_mask),
    dim=(d.nworld, m.ntendon, m.nv),
    inputs=[mask, m.dof_parentid, m.dof_Madr, m.tendon_armature, m.is_sparse, d.ten_J],
    outputs=[d.qM],
  )


@cache_kernel
def _copy_CSR(use_mask: bool):
  @wp.kernel(module="unique", enable_backward=False)
  def kernel(
    mask_in: wp.array(dtype=bool),
    # Model:
...
def _factor_i_sparse(m: Model, d: Data, M: wp.array3d(dtype=float), L: wp.array3d(dtype=float), D: wp.array2d(dtype=float), use_mask=False, mask=None):
  """Sparse L'*D*L factorization of inertia-like matrix M, assumed spd."""
  wp.launch(_copy_CSR(use_mask), dim=(d.nworld, m.nC), inputs=[mask, m.mapM2M, M], outputs=[L])

  for i in reversed(range(len(m.qLD_updates))):
    qLD_updates = m.qLD_updates[i]
    wp.launch(_qLD_acc(use_mask), dim=(d.nworld, qLD_updates.size), inputs=[mask, m.M_rownnz, m.M_rowadr, qLD_updates, L], outputs=[L])

  wp.launch(_qLDiag_div(use_mask), dim=(d.nworld, m.nv), inputs=[mask, m.M_rownnz, m.M_rowadr, L], outputs=[D])


@cache_kernel
def _tile_cholesky_factorize(tile: TileSet, use_mask: bool):
...
def _factor_i_dense(m: Model, d: Data, M: wp.array, L: wp.array, use_mask=False, mask=None):
  """Dense Cholesky factorization of inertia-like matrix M, assumed spd."""
  for tile in m.qM_tiles:
    wp.launch_tiled(
      _tile_cholesky_factorize(tile, use_mask),
      dim=(d.nworld, tile.adr.size),
      inputs=[mask, M, tile.adr],
      outputs=[L],
...
@event_scope
def factor_m(m: Model, d: Data, mask=None):
  """Factorization of inertia-like matrix M, assumed spd."""
  use_mask, mask = mask_args(mask)
  if m.is_sparse:
    _factor_i_sparse(m, d, d.qM, d.qLD, d.qLDiagInv, use_mask=use_mask, mask=mask)
  else:
    _factor_i_dense(m, d, d.qM, d.qLD, use_mask=use_mask, mask=mask)


@cache_kernel
def _cacc_world(use_mask: bool):
  @wp.kernel(module="unique", enable_backward=False)
  def kernel(
    mask_in: wp.array(dtype=bool),
    # In:
...
def _rne_cacc_world(m: Model, d: Data, use_mask=False, mask=None):
  if m.opt.disableflags & DisableBit.GRAVITY:
    if use_mask:
      wp.launch(
        support.masked_zero_spatial_vector(True),
        dim=d.cacc.shape,
        inputs=[mask],
        outputs=[d.cacc],
      )
    ...
    wp.launch(_cacc_world(use_mask), dim=[d.nworld], inputs=[mask, m.opt.gravity], outputs=[d.cacc])


@cache_kernel
def _cacc_branch(use_mask: bool):
  @wp.kernel(module="unique", enable_backward=False)
  def kernel(
    mask_in: wp.array(dtype=bool),
    # Model:
...
def _rne_cacc_forward(m: Model, d: Data, flg_acc: bool = False, use_mask=False, mask=None):
  wp.launch(
    _cacc_branch(use_mask),
    dim=(d.nworld, m.nbranch),
    inputs=[
      mask,
      m.body_parentid,
...
def _rne_cfrc(m: Model, d: Data, flg_cfrc_ext: bool = False, use_mask=False, mask=None):
  wp.launch(
    _cfrc(use_mask), dim=[d.nworld, m.nbody - 1], inputs=[mask, d.cinert, d.cvel, d.cacc, d.cfrc_ext, flg_cfrc_ext], outputs=[d.cfrc_int]
  )


@cache_kernel
def _cfrc_backward(use_mask: bool):
  @wp.kernel(module="unique", enable_backward=False)
  def kernel(
    mask_in: wp.array(dtype=bool),
    # Model:
...
def _rne_cfrc_backward(m: Model, d: Data, use_mask=False, mask=None):
  for body_tree in reversed(m.body_tree):
    wp.launch(
      _cfrc_backward(use_mask), dim=[d.nworld, body_tree.size], inputs=[mask, m.body_parentid, d.cfrc_int, body_tree], outputs=[d.cfrc_int]
    )


@cache_kernel
def _qfrc_bias(use_mask: bool):
  @wp.kernel(module="unique", enable_backward=False)
  def kernel(
    mask_in: wp.array(dtype=bool),
    # Model:
...
@event_scope
def rne(m: Model, d: Data, flg_acc: bool = False, mask=None):
  ...
  """
  use_mask, mask = mask_args(mask)
  _rne_cacc_world(m, d, use_mask=use_mask, mask=mask)
  _rne_cacc_forward(m, d, flg_acc=flg_acc, use_mask=use_mask, mask=mask)
  _rne_cfrc(m, d, use_mask=use_mask, mask=mask)
  _rne_cfrc_backward(m, d, use_mask=use_mask, mask=mask)
  wp.launch(_qfrc_bias(use_mask), dim=[d.nworld, m.nv], inputs=[mask, m.dof_bodyid, d.cdof, d.cfrc_int], outputs=[d.qfrc_bias])


@cache_kernel
def _cfrc_ext(use_mask: bool):
  @wp.kernel(module="unique", enable_backward=False)
  def kernel(
    mask_in: wp.array(dtype=bool),
    # Model:
...
@cache_kernel
def _count_equality_constraints(use_mask: bool):
  @wp.kernel(module="unique", enable_backward=False)
  def kernel(
    mask_in: wp.array(dtype=bool),
    # Model:
...
@cache_kernel
def _cfrc_ext_equality(use_mask: bool):
  @wp.kernel(module="unique", enable_backward=False)
  def kernel(
    mask_in: wp.array(dtype=bool),
    # Model:
...
@cache_kernel
def _cfrc_ext_contact(use_mask: bool):
  @wp.kernel(module="unique", enable_backward=False)
  def kernel(
    mask_in: wp.array(dtype=bool),
    # Model:
...
@event_scope
def rne_postconstraint(m: Model, d: Data, mask=None):
  ...
  wp.launch(
    _cfrc_ext(use_mask),
    dim=(d.nworld, m.nbody),
    inputs=[mask, m.body_rootid, d.xfrc_applied, d.xipos, d.subtree_com],
    outputs=[d.cfrc_ext],
  )
  ...
  if m.neq > 0:
    ...
    wp.launch(
      _count_equality_constraints(use_mask),
      dim=(d.nworld, d.njmax),  # TODO(team): launch over max equality constraints
      inputs=[mask, m.eq_type, d.ne, d.efc.type, d.efc.id],
      outputs=[ne_connect, ne_weld],
    )

    wp.launch(
      _cfrc_ext_equality(use_mask),
      dim=(d.nworld, m.neq),
      inputs=[
        mask,
        m.body_rootid,
  ...
  _rne_cacc_world(m, d, use_mask=use_mask, mask=mask)
  _rne_cacc_forward(m, d, flg_acc=True, use_mask=use_mask, mask=mask)

  # cfrc_body = cinert * cacc + cvel x (cinert * cvel)
  _rne_cfrc(m, d, flg_cfrc_ext=True, use_mask=use_mask, mask=mask)
...
@cache_kernel
def _tendon_bias_coef(use_mask: bool):
  @wp.kernel(module="unique", enable_backward=False)
  def kernel(
    mask_in: wp.array(dtype=bool),
    # Model:
...
@cache_kernel
def _tendon_bias_qfrc(use_mask: bool):
  @wp.kernel(module="unique", enable_backward=False)
  def kernel(
    mask_in: wp.array(dtype=bool),
    # Model:
...
@event_scope
def tendon_bias(m: Model, d: Data, qfrc: wp.array2d(dtype=float), mask=None):
  ...
  wp.launch(
    _tendon_bias_coef(use_mask),
    dim=(d.nworld, m.ntendon, m.nv),
    inputs=[mask, m.tendon_armature, d.qvel, ten_Jdot],
    outputs=[ten_bias_coef],
  )

  wp.launch(
    _tendon_bias_qfrc(use_mask),
    dim=(d.nworld, m.ntendon, m.nv),
    inputs=[mask, m.tendon_armature, d.ten_J, ten_bias_coef],
    outputs=[qfrc],
  )


@cache_kernel
def _comvel_root(use_mask: bool):
  @wp.kernel(module="unique", enable_backward=False)
  def kernel(
    mask_in: wp.array(dtype=bool),
...
@cache_kernel
def _comvel_branch(use_mask: bool):
  @wp.kernel(module="unique", enable_backward=False)
  def kernel(
    mask_in: wp.array(dtype=bool),
    # Model:
...
@event_scope
def com_vel(m: Model, d: Data, mask=None):
  ...
  """
  use_mask, mask = mask_args(mask)
  wp.launch(_comvel_root(use_mask), dim=(d.nworld, 6), inputs=[mask], outputs=[d.cvel])

  wp.launch(
    _comvel_branch(use_mask),
    dim=(d.nworld, m.nbranch),
    inputs=[
      mask,
      m.body_parentid,
...
@cache_kernel
def _transmission(use_mask: bool):
  @wp.kernel(module="unique", enable_backward=False)
  def kernel(
    mask_in: wp.array(dtype=bool),
    # Model:
...
@cache_kernel
def _transmission_body_moment(use_mask: bool):
  @wp.kernel(module="unique", enable_backward=False)
  def kernel(
    mask_in: wp.array(dtype=bool),
    # Model:
...
@cache_kernel
def _transmission_body_moment_scale(use_mask: bool):
  @wp.kernel(module="unique", enable_backward=False)
  def kernel(
    mask_in: wp.array(dtype=bool),
    # Model:
...
@event_scope
def transmission(m: Model, d: Data, mask=None):
  ...
  if m.nacttrnbody:
    ...
    wp.launch(
      _transmission_body_moment(use_mask),
    ...
    # scale moments
    wp.launch(
      _transmission_body_moment_scale(use_mask),
      dim=(d.nworld, m.nacttrnbody, m.nv),
      inputs=[mask, m.actuator_trntype_body_adr, ncon],
      outputs=[d.actuator_moment],
    )


@cache_kernel
def _solve_LD_sparse_x_acc_up(use_mask: bool):
  @wp.kernel(module="unique", enable_backward=False)
  def kernel(
    mask_in: wp.array(dtype=bool),
    # In:
...
@cache_kernel
def _solve_LD_sparse_qLDiag_mul(use_mask: bool):
  @wp.kernel(module="unique", enable_backward=False)
  def kernel(
    mask_in: wp.array(dtype=bool),
    # In:
...
@cache_kernel
def _solve_LD_sparse_x_acc_down(use_mask: bool):
  @wp.kernel(module="unique", enable_backward=False)
  def kernel(
    mask_in: wp.array(dtype=bool),
    # In:
...
def _solve_LD_sparse(
  ...
  for qLD_updates in reversed(m.qLD_updates):
    wp.launch(_solve_LD_sparse_x_acc_up(use_mask), dim=(d.nworld, qLD_updates.size), inputs=[mask, L, qLD_updates], outputs=[x])

  wp.launch(_solve_LD_sparse_qLDiag_mul(use_mask), dim=(d.nworld, m.nv), inputs=[mask, D], outputs=[x])

  for qLD_updates in m.qLD_updates:
    wp.launch(_solve_LD_sparse_x_acc_down(use_mask), dim=(d.nworld, qLD_updates.size), inputs=[mask, L, qLD_updates], outputs=[x])


@cache_kernel
def _tile_cholesky_solve(tile: TileSet, use_mask: bool):
...
def _solve_LD_dense(m: Model, d: Data, L: wp.array3d(dtype=float), x: wp.array2d(dtype=float), y: wp.array2d(dtype=float), use_mask=False, mask=None):
  """Computes dense backsubstitution: x = inv(L'*L)*y."""
  for tile in m.qM_tiles:
    wp.launch_tiled(
      _tile_cholesky_solve(tile, use_mask),
      dim=(d.nworld, tile.adr.size),
      inputs=[mask, L, y, tile.adr],
      outputs=[x],
...
def solve_LD(
  ...
  use_mask, mask = mask_args(mask)
  if m.is_sparse:
    _solve_LD_sparse(m, d, L, D, x, y, use_mask=use_mask, mask=mask)
  else:
    _solve_LD_dense(m, d, L, x, y, use_mask=use_mask, mask=mask)


@event_scope
def solve_m(m: Model, d: Data, x: wp.array2d(dtype=float), y: wp.array2d(dtype=float), mask=None):
  ...
  """
  use_mask, mask = mask_args(mask)
  solve_LD(m, d, d.qLD, d.qLDiagInv, x, y, mask=mask)


@cache_kernel
def _tile_cholesky_factorize_solve(tile: TileSet, use_mask: bool):
...
def _factor_solve_i_dense(
  ...
  for tile in m.qM_tiles:
    wp.launch_tiled(
      _tile_cholesky_factorize_solve(tile, use_mask),
      dim=(d.nworld, tile.adr.size),
      inputs=[mask, M, y, tile.adr],
      outputs=[x, L],
      block_dim=m.block_dim.cholesky_factorize_solve,
    )


def factor_solve_i(m, d, M, L, D, x, y, mask=None):
  ...
  use_mask, mask = mask_args(mask)
  if m.is_sparse:
    _factor_i_sparse(m, d, M, L, D, use_mask=use_mask, mask=mask)
    _solve_LD_sparse(m, d, L, D, x, y, use_mask=use_mask, mask=mask)
  else:
    _factor_solve_i_dense(m, d, M, x, y, L, use_mask=use_mask, mask=mask)


@cache_kernel
def _subtree_vel_forward(use_mask: bool):
  @wp.kernel(module="unique", enable_backward=False)
  def kernel(
    mask_in: wp.array(dtype=bool),
    # Model:
...
@cache_kernel
def _linear_momentum(use_mask: bool):
  @wp.kernel(module="unique", enable_backward=False)
  def kernel(
    mask_in: wp.array(dtype=bool),
    # Model:
...
@cache_kernel
def _angular_momentum(use_mask: bool):
  @wp.kernel(module="unique", enable_backward=False)
  def kernel(
    mask_in: wp.array(dtype=bool),
    # Model:
...
def subtree_vel(m: Model, d: Data, mask=None):
  ...
  # bodywise quantities
  wp.launch(
    _subtree_vel_forward(use_mask),
    dim=(d.nworld, m.nbody),
    inputs=[mask, m.body_rootid, m.body_mass, m.body_inertia, d.xipos, d.ximat, d.subtree_com, d.cvel],
  ...
  for body_tree in reversed(m.body_tree):
    wp.launch(
      _linear_momentum(use_mask),
      dim=[d.nworld, body_tree.size],
      inputs=[mask, m.body_parentid, m.body_subtreemass, d.subtree_linvel, body_tree],
      outputs=[d.subtree_linvel],
    )

  for body_tree in reversed(m.body_tree):
    wp.launch(
      _angular_momentum(use_mask),
      dim=[d.nworld, body_tree.size],
      inputs=[
        mask,
        m.body_parentid,
...
@cache_kernel
def _joint_tendon(use_mask: bool):
  @wp.kernel(module="unique", enable_backward=False)
  def kernel(
    mask_in: wp.array(dtype=bool),
    # Model:
...
@cache_kernel
def _spatial_site_tendon(use_mask: bool):
  @wp.kernel(module="unique", enable_backward=False)
  def kernel(
    mask_in: wp.array(dtype=bool),
    # Model:
...
@cache_kernel
def _spatial_geom_tendon(use_mask: bool):
  @wp.kernel(module="unique", enable_backward=False)
  def kernel(
    mask_in: wp.array(dtype=bool),
    # Model:
...
@cache_kernel
def _spatial_tendon_wrap(use_mask: bool):
  @wp.kernel(module="unique", enable_backward=False)
  def kernel(
    mask_in: wp.array(dtype=bool),
    # Model:
...
def tendon(m: Model, d: Data, mask=None):
  ...
  if use_mask:
    wp.launch(
      support.masked_zero_2d(True),
      dim=d.ten_length.shape,
      inputs=[mask],
      outputs=[d.ten_length],
    )
    wp.launch(
      support.masked_zero_3d(True),
      dim=d.ten_J.shape,
  ...
  wp.launch(
    _joint_tendon(use_mask),
    dim=(d.nworld, m.wrap_jnt_adr.size),
    inputs=[mask, m.jnt_qposadr, m.jnt_dofadr, m.wrap_objid, m.wrap_prm, m.tendon_jnt_adr, m.wrap_jnt_adr, d.qpos],
  ...
  if spatial_site or spatial_geom:
    wp.launch(
      _spatial_tendon_wrap(use_mask),
      dim=d.nworld,
      inputs=[mask, m.ntendon, m.tendon_adr, m.tendon_num, m.wrap_type, m.wrap_objid, d.site_xpos, wrap_geom_xpos],
