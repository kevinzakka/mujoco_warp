...
@cache_kernel
def _spring_damper_dof_passive(use_mask: bool):
...
@cache_kernel
def _spring_damper_tendon_passive(use_mask: bool):
  @wp.kernel(module="unique", enable_backward=False)
  def kernel(
    mask_in: wp.array(dtype=bool),
    # Model:
...
@cache_kernel
def _gravity_force(use_mask: bool):
  @wp.kernel(module="unique", enable_backward=False)
  def kernel(
    mask_in: wp.array(dtype=bool),
    # Model:
...
@cache_kernel
def _fluid_force(use_mask: bool):
  @wp.kernel(module="unique", enable_backward=False)
  def kernel(
    mask_in: wp.array(dtype=bool),
    # Model:
...
@cache_kernel
def _qfrc_passive(use_mask: bool):
  @wp.kernel(module="unique", enable_backward=False)
  def kernel(
    mask_in: wp.array(dtype=bool),
    # Model:
...
@cache_kernel
def _flex_elasticity(use_mask: bool):
  @wp.kernel(module="unique", enable_backward=False)
  def kernel(
    mask_in: wp.array(dtype=bool),
    # Model:
...
@cache_kernel
def _flex_bending(use_mask: bool):
  @wp.kernel(module="unique", enable_backward=False)
  def kernel(
    mask_in: wp.array(dtype=bool),
    # Model:
...
@event_scope
def passive(m: Model, d: Data, mask=None):
  ...
  if dsbl_spring and dsbl_damper:
    if use_mask:
      wp.launch(
        ...
        dim=d.qfrc_spring.shape,
        inputs=[mask],
        outputs=[d.qfrc_spring],
      )
      wp.launch(
        support.masked_zero_2d(True),
        dim=d.qfrc_damper.shape,
        inputs=[mask],
        outputs=[d.qfrc_damper],
      )
      wp.launch(
        support.masked_zero_2d(True),
        dim=d.qfrc_gravcomp.shape,
        inputs=[mask],
        outputs=[d.qfrc_gravcomp],
      )
      wp.launch(
        support.masked_zero_2d(True),
        dim=d.qfrc_fluid.shape,
        inputs=[mask],
        outputs=[d.qfrc_fluid],
      )
      wp.launch(
        support.masked_zero_2d(True),
        dim=d.qfrc_passive.shape,
        inputs=[mask],
  ...
  wp.launch(
    _spring_damper_dof_passive(use_mask),
    dim=(d.nworld, m.njnt),
    inputs=[
      mask,
      m.opt.disableflags,
  ...
  if m.ntendon:
    wp.launch(
      _spring_damper_tendon_passive(use_mask),
  ...
  if m.has_fluid:
    _fluid(m, d, use_mask, mask)

  wp.launch(
    _qfrc_passive(use_mask),
    dim=(d.nworld, m.nv),
    inputs=[
      mask,
      m.jnt_actgravcomp,
