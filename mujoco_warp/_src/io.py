...
class PerWorldMeshResult:
  """Structured output from ``per_world_mesh``.

  Attributes:
    model: Warp model with per-world ``geom_dataid`` and dependent fields.
    host_model: Padded host MjModel matching the Warp model layout.
    candidate_mesh_ids: All mesh IDs referenced by any variant. Pass to
      ``create_render_context(preload_mesh_ids=...)`` so all candidates
      get BVHs.
    real_geom_mask: Boolean array of shape ``(ngeom,)``. ``True`` for geoms
      that exist in the original model, ``False`` for padding geoms added
      for body-level variants with different geom counts.
    dataid_table: Integer array of shape ``(nworld, ngeom)`` with the
      current per-world mesh assignments.
    variant_keys: Integer array of shape ``(n_unique, ngeom)`` listing
      each unique dataid row that was compiled at build time.
    variant_fields: Dict mapping field names to arrays of shape
      ``(n_unique, ...)``. The *i*-th slice contains the compiled
      dependent field values for ``variant_keys[i]``.
  """

  model: types.Model
  host_model: mujoco.MjModel
  candidate_mesh_ids: np.ndarray
...
def _compile_variants(spec, padded_model, dataid_table, nworld, geom_variants, body_variants):
  """Compile each unique variant and return per-world field arrays + cache.

  Returns ``(field_arrays, variant_cache)`` where:
    - ``field_arrays`` maps field names to ``(nworld, ...)`` numpy arrays.
    - ``variant_cache`` maps dataid-row tuples to single-world field dicts.
  ...
  if len(unique_rows) <= 1:
    # All worlds are the same; build a trivial cache from the padded model.
    key = tuple(dataid_table[0])
    ngeom = padded_model.ngeom
    nbody = padded_model.nbody
    single = {
      "geom_size": padded_model.geom_size.copy(),
      "geom_rbound": padded_model.geom_rbound.copy(),
      "geom_aabb": padded_model.geom_aabb.reshape(ngeom, 2, 3).copy(),
      "geom_pos": padded_model.geom_pos.copy(),
      "body_mass": padded_model.body_mass.copy(),
      "body_subtreemass": padded_model.body_subtreemass.copy(),
      "body_inertia": padded_model.body_inertia.copy(),
      "body_invweight0": padded_model.body_invweight0.copy(),
      "body_ipos": padded_model.body_ipos.copy(),
      "body_iquat": padded_model.body_iquat.copy(),
    }
    return {}, {key: single}

  # Save spec state (index-based to handle unnamed geoms).
  ...
  # Build per-world arrays and per-variant cache.
  ngeom = padded_model.ngeom
  nbody = padded_model.nbody
  ...
  variant_cache = {}
  for key, ref in compiled_variants.items():
    variant_cache[key] = {
      "geom_size": ref.geom_size.copy(),
      "geom_rbound": ref.geom_rbound.copy(),
      "geom_aabb": ref.geom_aabb.reshape(ngeom, 2, 3).copy(),
      "geom_pos": ref.geom_pos.copy(),
      "body_mass": ref.body_mass.copy(),
      "body_subtreemass": ref.body_subtreemass.copy(),
      "body_inertia": ref.body_inertia.copy(),
      "body_invweight0": ref.body_invweight0.copy(),
      "body_ipos": ref.body_ipos.copy(),
      "body_iquat": ref.body_iquat.copy(),
    }
...
def per_world_mesh(spec: mujoco.MjSpec, nworld: int) -> PerWorldMeshResult:
  ...
  padded geoms, and a variant cache for runtime reassignment.
  """
  model = spec.compile()
  ...
  for tuple_id in range(model.ntuple):
    ...
    for i in range(size):
      ...
      for j in range(var_size):
        if model.tuple_objtype[var_start + j] == mujoco.mjtObj.mjOBJ_MESH:
          var_meshes.append(model.tuple_objid[var_start + j])
          all_mesh_ids.add(int(model.tuple_objid[var_start + j]))
    ...
    prm_candidates = [(0, prm) for _, prm in variants]
  ...
  if not geom_variants and not body_variants:
    return PerWorldMeshResult(
      ...
      variant_keys=np.array([base_dataid], dtype=int),
      variant_fields={},
    )

  m.geom_dataid = wp.array(dataid_table, dtype=int)

  field_arrays, variant_cache = _compile_variants(spec, model, dataid_table, nworld, geom_variants, body_variants)
  ...
  # Build variant_keys and variant_fields arrays for reassignment.
